...
// Recent projects history (stored in chrome.storage.local)
const RECENT_PROJECTS_KEY = 'recentProjects';
const MAX_RECENT_PROJECTS = 5;

async function saveRecentProject(project) {
  try {
    const result = await chrome.storage.local.get(RECENT_PROJECTS_KEY);
    const projects = result[RECENT_PROJECTS_KEY] || [];
    // Remove duplicate if already in list
    const filtered = projects.filter(p => p.id !== project.id);
    filtered.unshift({ id: project.id, name: project.name, timestamp: Date.now() });
    if (filtered.length > MAX_RECENT_PROJECTS) {
      filtered.length = MAX_RECENT_PROJECTS;
    }
    await chrome.storage.local.set({ [RECENT_PROJECTS_KEY]: filtered });
  } catch (error) {
    console.error('[Utils] Error saving recent project:', error);
  }
}
...
async function getRecentProjects() {
  try {
    ...
    console.error('[Utils] Error loading recent projects:', error);
    return [];
  }
}
