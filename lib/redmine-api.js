...
class RedmineAPI {
  ...
  async createIssue(issueData) {
    ...
    // Try creating issue without uploads first to test if basic creation works
    if (sanitizedIssueData.uploads && sanitizedIssueData.uploads.length > 0) {
      console.log('[Redmine API] Testing: Creating issue WITHOUT attachments first...');
      const testDataWithoutUploads = { ...sanitizedIssueData };
      delete testDataWithoutUploads.uploads;

      try {
        const testIssue = await this.request('/issues.json', {
          method: 'POST',
          body: JSON.stringify({ issue: testDataWithoutUploads })
        });
        console.log('[Redmine API] ✅ SUCCESS! Issue created without attachments: #' + testIssue.issue.id);
        console.log('[Redmine API] This means the uploads array is causing the 500 error');
        console.log('[Redmine API] Deleting test issue #' + testIssue.issue.id);

        // Delete the test issue
        await this.request(`/issues/${testIssue.issue.id}.json`, { method: 'DELETE' });
        console.log('[Redmine API] Test issue deleted');

      } catch (testError) {
        console.error('[Redmine API] ❌ FAILED! Issue creation without attachments also fails');
        console.error('[Redmine API] This means the problem is NOT the uploads array');
        console.error('[Redmine API] Error:', testError.message);
        console.error('[Redmine API] Likely causes: required custom fields, workflow validation, permissions');
      }
    }
